...
public abstract class UrlClient {
    ...
    private async Task<ClientForUrl> Create(CancellationToken token) {
        ...
        var resolvedUri = resolve
            ? await UrlHost.Resolve(uri, token)
            : uri;
        if (Log.Info()) {
        ...
        if (credential != null) {
            ...
            if (password != null && password != "") {
                ...
                client.Password(await credential.UnprotectPassword(token));
            }
        }
        Configure(client);
        return client;
    ...
    private protected async Task<UrlListing<T>> List<T>(UrlListingParser<T> parser, CancellationToken token) {
        ...
        var stream = await Read(null, token);
        ...
        async IAsyncEnumerable<string> contents() {
            await foreach (var item in streamTextBuilder.Read(token)) {
                if (item.Clear == false) {
        ...
        async IAsyncEnumerable<T> list() {
            await foreach (var item in parser.ParseContents(contents(), token)) {
                yield return item;
            }
            await streamTextTask;
        }
