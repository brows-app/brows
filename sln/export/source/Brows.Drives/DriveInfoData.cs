...
internal static class DriveInfoData {
    ...
    public sealed class Icon : Image {
        protected sealed override async Task<object> GetValue(DriveEntry entry,
    ...
    public abstract class Image : EntryDataExport<DriveEntry, object> {
    }
}

internal abstract class DriveInfoData<T> : EntryDataExport<DriveEntry, T> {
    ...
    protected override async Task<T> GetValue(DriveEntry entry, Action<T> progress, CancellationToken token) {
        await entry.RefreshInternal(token);
