...
internal sealed class FileSystemProviderFactory : ProviderFactory<FileSystemProvider>,
    ...
    protected sealed override async Task<FileSystemProvider> CreateFor(string id,
                                                                       IPanel panel,
        ...
        await Task
            .WhenAll(
                CaseCorrectFile?.Work(info.FullName, result => caseCorrect = result, token)
                    ?? Task.CompletedTask,
                CaseSensitiveDirectory?.Work(info.FullName, result => caseSensitive = result, token)
                    ?? Task.CompletedTask);
        if (Log.Info()) {
            Log.Info(
