...
internal sealed class FileSystemProvider : Provider<FileSystemEntry, FileSystemConfig>,
    ...
    private async Task ParentEventTask(FileSystemEventArgs e, CancellationToken token) {
        ...
        switch (e.ChangeType) {
            case WatcherChangeTypes.Renamed:
                ...
                if (oldName != null) {
                    var isThisDir = Directory.Name.Equals(oldName, CaseSensitive
            ...
            default:
                var name = e.Name;
                if (name != null) {
                    var isThisDir = Directory.Name.Equals(name, CaseSensitive
                        ? StringComparison.Ordinal
                        : StringComparison.OrdinalIgnoreCase);
                    if (isThisDir) {
    ...
    protected sealed override async Task<bool> Drop(IPanelDrop data,
                                                    IOperationProgress operationProgress,
    ...
    protected sealed override async Task Begin(CancellationToken token) {
        ...
        ParentEvent = Parent != null &&
                      Parent != Drives.ID &&
                      (await FileSystemTask.ExistingDirectory(Parent, token) != null)
    ...
    public FileSystemProvider(FileSystemProviderFactory factory,
    ...
    internal sealed class ProvideIO : IProvideIO, IProviderExport<FileSystemProvider> {
        public async Task<bool> Work(ICollection<IProvidedIO> io,
    ...
    private sealed class MoveProvidedIO : IMoveProvidedIO, IProviderExport<FileSystemProvider> {
        public async Task<bool> Work(IEnumerable<IProvidedIO> io,
                                     IProvider target,
                                     IOperationProgress progress,
                                     CancellationToken token) {
            if (io is null) throw new ArgumentNullException(nameof(io));
    ...
    internal sealed class CopyProvidedIO : ICopyProvidedIO, IProviderExport<FileSystemProvider> {
        public async Task<bool> Work(IEnumerable<IProvidedIO> io,
                                     IProvider target,
                                     IOperationProgress progress,
                                     CancellationToken token) {
            ...
            await Task.WhenAll(streams.Select(set =>
                set.Consume(progress: progress, token: token, consuming: async (source, stream, progress, token) => {
                    ...
                    await progress.Child(relativePath, OperationProgressKind.FileSize, async (progress, token) => {
                        await using var destination = await Task.Run(fileInfo.Create, token);
    ...
    private sealed class Bookmark : IBookmark, IProviderExport<FileSystemProvider> {
        public async Task<bool> Work(IReadOnlyList<KeyValuePair<string, string>> existing,
            ...
            for (; ; ) {
                ...
                if (conflicts.Any()) {
                    var alreadyBookmarked = conflicts.Any(item => dir.Equals(item.Value, provider.CaseSensitive
                        ? StringComparison.Ordinal
                        : StringComparison.OrdinalIgnoreCase));
