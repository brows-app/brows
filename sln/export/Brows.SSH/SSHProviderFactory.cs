...
namespace Brows {
    internal sealed class SSHProviderFactory : ProviderFactory<Provider> {
        ...
        protected sealed override async Task<Provider> CreateFor(string id, IPanel panel, CancellationToken token) {
            ...
            var nativeLoaded = await Native.Loaded(token).ConfigureAwait(false);
            if (nativeLoaded == false) {
                return null;
            }
            var icon = await IconCache.Ready(token).ConfigureAwait(false);
            var client = await Client(uri, token).ConfigureAwait(false);
