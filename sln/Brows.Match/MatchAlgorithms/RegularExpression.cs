...
namespace Brows.MatchAlgorithms {
    internal class RegularExpression : MatchAlgorithm {
        ...
        public sealed override Matcher Matcher(string pattern) {
            var regex = Convert(pattern);
            return new RegularExpressionMatcher(regex);
        }

        public sealed override async IAsyncEnumerable<T> Match<T>(string pattern, IEnumerable<T> source, Func<T, string> selector, [EnumeratorCancellation] CancellationToken cancellationToken) {
