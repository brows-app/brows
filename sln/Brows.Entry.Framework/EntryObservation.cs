...
namespace Brows {
    internal abstract class EntryObservation : Notifier, IEntryObservation, IControlled<IEntryObservationController> {
            ...
            foreach (var chunk in chunks) {
                ...
                if (sorting.Count > 0) {
                    var sortingTasks = chunk
                        ...
                        .ToList();
                    if (sortingTasks.Any(task => task.IsCompleted == false)) {
                        added = sortingTasks.Select(async task => {
                            var item = await task;
                            Collection.Add(item);
                        });
                ...
                if (added == null) {
                    using (Controller?.Updating()) {
                        Collection.Add(chunk.Where(item => item is not null).Cast<IEntry>());
                    }
                }
        ...
        private EntryProvider _Provider;
